...
class RowBatchSerializeBaseline {
 public:
  ...
  static int Serialize(RowBatch* batch, ProtoRowBatch* output_batch) {
    ...
    for (auto desc: batch->row_desc().tuple_descriptors()) {
    ...
    if (size > 0) {
      ...
      uint8_t* input = output_batch->tuple_data.mutable_data();
      ...
      if (LIKELY(compressed_size < size)) {
        batch->compression_scratch_.resize(compressed_size);
        output_batch->TransferTupleData(&batch->compression_scratch_);
  ...
  static void SerializeInternal(
      RowBatch* batch, int64_t size, ProtoRowBatch* output_batch) {
    string serialized;
    serialized.resize(size);
    output_batch->header.set_uncompressed_size(size);
    output_batch->tuple_offsets.reserve(batch->num_rows() * batch->num_tuples_per_row());
    ...
    char* tuple_data = const_cast<char*>(serialized.c_str());
    ...
    output_batch->TransferTupleData(&serialized);
  ...
  // Copy of baseline version without dedup logic
  static void Deserialize(RowBatch* batch, const ProtoRowBatch& input_batch) {
...
class RowBatchSerializeBenchmark {
 public:
  ...
  static void Run() {
    ...
    ProtoRowBatch adjacent_dup_tbatch;
