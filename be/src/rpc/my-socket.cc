...
MySocket::MySocket(int port, int sendTimeout, int recvTimeout) :
  port_(port),
  serverSocket_(-1),
  acceptBacklog_(1),
...
void MySocket::setSendTimeout(int sendTimeout) {
  sendTimeout_ = sendTimeout;
}

void MySocket::setRecvTimeout(int recvTimeout) {
  recvTimeout_ = recvTimeout;
}

void MySocket::setAcceptTimeout(int accTimeout) {
  accTimeout_ = accTimeout;
}

void MySocket::setRetryLimit(int retryLimit) {
  retryLimit_ = retryLimit;
}

void MySocket::setRetryDelay(int retryDelay) {
  retryDelay_ = retryDelay;
}

void MySocket::setTcpSendBuffer(int tcpSendBuffer) {
  tcpSendBuffer_ = tcpSendBuffer;
}

void MySocket::setTcpRecvBuffer(int tcpRecvBuffer) {
...
void MySocket::listen() {
  ...
  if (-1 == socketpair(AF_LOCAL, SOCK_STREAM, 0, sv)) {
    GlobalOutput.perror("MySocket::listen() socketpair() ", errno);
    intSock1_ = -1;
    intSock2_ = -1;
  } else {
    intSock1_ = sv[1];
  ...
  if (serverSocket_ == -1) {
    ...
    GlobalOutput.perror("MySocket::listen() socket() ", errno_copy);
    close();
  ...
  if (-1 == setsockopt(serverSocket_, SOL_SOCKET, SO_REUSEADDR,
    ...
    int errno_copy = errno;
    GlobalOutput.perror("MySocket::listen() setsockopt() SO_REUSEADDR ", errno_copy);
    close();
  ...
  if (tcpSendBuffer_ > 0) {
    if (-1 == setsockopt(serverSocket_, SOL_SOCKET, SO_SNDBUF,
      ...
      int errno_copy = errno;
      GlobalOutput.perror("MySocket::listen() setsockopt() SO_SNDBUF ", errno_copy);
      close();
  ...
  if (tcpRecvBuffer_ > 0) {
    if (-1 == setsockopt(serverSocket_, SOL_SOCKET, SO_RCVBUF,
      ...
      int errno_copy = errno;
      GlobalOutput.perror("MySocket::listen() setsockopt() SO_RCVBUF ", errno_copy);
      close();
  ...
  if (-1 == setsockopt(serverSocket_, SOL_SOCKET, TCP_DEFER_ACCEPT,
                       &one, sizeof(one))) {
    int errno_copy = errno;
    GlobalOutput.perror("MySocket::listen() setsockopt() TCP_DEFER_ACCEPT ", errno_copy);
    close();
  ...
  if (res->ai_family == AF_INET6 && path_.empty()) {
    ...
    if (-1 == setsockopt(serverSocket_, IPPROTO_IPV6, IPV6_V6ONLY,
      ...
      GlobalOutput.perror("MySocket::listen() IPV6_V6ONLY ", errno);
    }
  }
  ...
  if (-1 == setsockopt(serverSocket_, SOL_SOCKET, SO_LINGER,
    ...
    int errno_copy = errno;
    GlobalOutput.perror("MySocket::listen() setsockopt() SO_LINGER ", errno_copy);
    close();
  ...
  if (path_.empty()) {
    ...
    if (-1 == setsockopt(serverSocket_, IPPROTO_TCP, TCP_NODELAY,
                         cast_sockopt(&one), sizeof(one))) {
      int errno_copy = errno;
      GlobalOutput.perror("MySocket::listen() setsockopt() TCP_NODELAY ", errno_copy);
      close();
  ...
  if (flags == -1) {
    int errno_copy = errno;
    GlobalOutput.perror("MySocket::listen() fcntl() F_GETFL ", errno_copy);
  ...
  if (-1 == fcntl(serverSocket_, F_SETFL, flags | O_NONBLOCK)) {
    int errno_copy = errno;
    GlobalOutput.perror("MySocket::listen() fcntl() O_NONBLOCK ", errno_copy);
    throw TTransportException(TTransportException::NOT_OPEN, "fcntl() failed", errno_copy);
  }
  ...
  if (retries > retryLimit_) {
    char errbuf[1024];
    if (! path_.empty()) {
      sprintf(errbuf, "MySocket::listen() PATH %s", path_.c_str());
    }
    else {
      sprintf(errbuf, "MySocket::listen() BIND %d", port_);
    }
    GlobalOutput(errbuf);
    close();
  ...
  if (-1 == ::listen(serverSocket_, acceptBacklog_)) {
    int errno_copy = errno;
    GlobalOutput.perror("MySocket::listen() listen() ", errno_copy);
    close();
...
shared_ptr<TTransport> MySocket::acceptImpl() {
  if (serverSocket_ == -1) {
    throw TTransportException(TTransportException::NOT_OPEN, "MySocket not listening");
  }
  ...
  while (true) {
    ...
    if (ret < 0) {
      ...
      int errno_copy = errno;
      GlobalOutput.perror("MySocket::acceptImpl() poll() ", errno_copy);
      ...
      if (intSock2_ != -1 && (fds[1].revents & POLLIN)) {
        ...
        if (-1 == recv(intSock2_, cast_sockopt(&buf), sizeof(int8_t), 0)) {
          GlobalOutput.perror("MySocket::acceptImpl() recv() interrupt ", errno);
        }
      ...
      GlobalOutput("MySocket::acceptImpl() poll 0");
  ...
  if (clientSocket == -1) {
    int errno_copy = errno;
    GlobalOutput.perror("MySocket::acceptImpl() ::accept() ", errno_copy);
  ...
  if (flags == -1) {
    int errno_copy = errno;
    GlobalOutput.perror("MySocket::acceptImpl() fcntl() F_GETFL ", errno_copy);
  ...
  if (-1 == fcntl(clientSocket, F_SETFL, flags & ~O_NONBLOCK)) {
    int errno_copy = errno;
    GlobalOutput.perror("MySocket::acceptImpl() fcntl() F_SETFL ~O_NONBLOCK ", errno_copy);
...
shared_ptr<TSocket> MySocket::createSocket(SOCKET clientSocket) {
  return shared_ptr<TSocket>(new TSocket(clientSocket));
}

void MySocket::interrupt() {
  if (intSock1_ != -1) {
    int8_t byte = 0;
    if (-1 == send(intSock1_, cast_sockopt(&byte), sizeof(int8_t), 0)) {
      GlobalOutput.perror("MySocket::interrupt() send() ", errno);
    }
  }
}
