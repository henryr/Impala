...
class Rpc {
 public:
  ...
  /// Handles asynchronous execution, and retry, of a single RPC. The completion callback
  /// 'cb' is guaranteed to be called exactly once - whether the RPC was successful or
  /// not. The callback should have the following signature:
  ///
  ...
  ///
  /// 'req' and 'resp' are owned by the caller, and must not be destroyed before 'cb' is
  /// called. A convenient pattern is to allocate 'req' and 'resp' on the heap and delete
  /// them in 'cb'.
  ...
  void ExecuteWithThriftArgsAsync(
      const F& func, TREQ* req, TRESP* resp, const CB& cb) const {
    std::unique_ptr<ThriftWrapperPb> request_proto = std::make_unique<ThriftWrapperPb>();
    Status status = SerializeThriftToProtoWrapper(req, request_proto.get());
    if (!status.ok()) {
      cb(status, req, resp, nullptr);
      return;
    }
    ...
    ExecuteAsync(func, request_proto.release(), response_proto.release(),
        [ req, resp, cb = std::move(cb) ](const Status& status, ThriftWrapperPb* request,
                     ThriftWrapperPb* response, kudu::rpc::RpcController* controller) {

          if (status.ok() && controller->status().ok()) {
            cb(DeserializeThriftFromProtoWrapper(*response, resp), req, resp, controller);
          } else {
            cb(status, req, resp, controller);
          }
          delete request;
          delete response;
        });
