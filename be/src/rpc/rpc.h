...
class Rpc {
 public:
  ...
  Rpc& SetRetryInterval(int32_t interval_ms) {
    ...
    params_->retry_interval_ms = interval_ms;
    return *this;
  }
  ...
  /// 'cb' is guaranteed to be called exactly once after the invocation and any retries
  /// have finished - whether the RPC was successful or not. The callback should have the
  /// following signature:
  ///
  ...
  ///
  /// 'cb' will be called from a reactor thread, which are a finite resource that do much
  /// of the work of RPC processing. Therefore, 'cb' should not block for an extended
  /// period of time, and should do as little work as possible.
  ///
  ...
  void ExecuteAsync(const F& func, REQ* req, RESP* resp, const CB& cb) {
    Status status = CheckConfiguration();
    if (!status.ok()) {
      cb(status, req, resp, nullptr);
      return;
    }
    ExecuteAsyncHelper(func, req, resp, 0, cb, std::move(params_), mgr_);
  }
  ...
  Status Execute(const F& func, const REQ& req, RESP* resp) {
    ...
    for (int i = 0; i < params_->max_rpc_attempts; ++i) {
      ...
      SleepForMs(params_->retry_interval_ms);
    }
 ...
 private:
  struct RpcParams {
    ...
    int32_t retry_interval_ms = RPC_DEFAULT_RETRY_INTERVAL_MS;
  ...
  Status CheckConfiguration() const {
    ...
    if (params_->max_rpc_attempts > 1 && params_->retry_interval_ms <= 0) {
      return Status(
          strings::Substitute("Invalid retry interval: $0", params_->retry_interval_ms));
    }
  ...
  /// on a reactor thread.
  template <typename F, typename REQ, typename RESP, typename CB>
  static void ExecuteAsyncHelper(const F& func, REQ* req, RESP* resp, int num_attempts,
      CB cb, std::shared_ptr<RpcParams> params, RpcMgr* mgr) {
    ...
    //
    // The completion callback 'cb' is guaranteed to be called.
    //
    ...
    // synchronous case).
    auto cb_wrapper = [params = std::move(params), mgr, func, req, resp,
      ...
      kudu::MonoDelta retry_interval =
          kudu::MonoDelta::FromMilliseconds(params->retry_interval_ms);
      auto reactor_task = [params = std::move(params), mgr, func, req, resp,
          cb = std::move(cb), num_attempts](const kudu::Status& status) mutable {
        ...
        ExecuteAsyncHelper(func, req, resp, num_attempts, cb, std::move(params), mgr);
      };
      mgr->messenger()->ScheduleOnReactor(reactor_task, retry_interval);
    };

    ((proxy.get())->*func)(*req, resp, controller_ptr, cb_wrapper);
  }
};
