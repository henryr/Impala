...
std::string GetSaslProtoName() {
  DCHECK(FLAGS_server_require_kerberos);
...
Status AuthManager::InitKerberosEnv() {
  ...
  RETURN_IF_ERROR(FromKuduStatus(kudu::security::InitKerberosForServer()));
...
Status AuthManager::Init() {
  ...
  if (!kerberos_internal_principal.empty()) {
    // Notify KuduRPC to use kerberos by setting this flag.
    FLAGS_server_require_kerberos = true;
