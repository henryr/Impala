...
ProtoBloomFilter BfUnion(const BloomFilter& x, const BloomFilter& y, bool* success) {
  ProtoBloomFilter thrift_x, thrift_y;
...
TEST(BloomFilter, Thrift) {
  ...
  ProtoBloomFilter to_thrift;
  BloomFilter::ToProto(&bf, &to_thrift);
  EXPECT_EQ(to_thrift.header.always_true(), false);
  ...
  EXPECT_EQ(to_thrift.header.always_true(), true);
}
