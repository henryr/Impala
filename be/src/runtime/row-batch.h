...
/// on-the-wire form of the tuple data. Typically outbound ProtoRowBatches do not own the
/// tuple data (because it may be shared amongst many identical batches for different
/// destinations), where incoming ProtoRowBatches do own the tuple data which is copied
/// from the wire into the 'owned_tuple_data' buffer.
