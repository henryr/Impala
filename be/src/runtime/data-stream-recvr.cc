...
void DataStreamRecvr::SenderQueue::AddBatch(const TransmitDataCtx& payload) {
...

  {
    ...
    COUNTER_ADD(recvr_->good_bytes_received_counter_, batch_size);
...
DataStreamRecvr::DataStreamRecvr(DataStreamMgr* stream_mgr, MemTracker* parent_tracker,
  ...
  good_bytes_received_counter_ =
      ADD_COUNTER(recvr_side_profile, "AcceptedBytesReceived", TUnit::BYTES);
  bytes_received_time_series_counter_ =
      ADD_TIME_SERIES_COUNTER(recvr_side_profile, "BytesReceived", bytes_received_counter_);
