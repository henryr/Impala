...
void Coordinator::BackendState::Exec(
  ...
  Status rpc_status = backend_client.DoRpc(
      &ImpalaInternalServiceClient::ExecQueryFInstances, rpc_params, &thrift_result);
  rpc_sent_ = true;
...
bool Coordinator::BackendState::Cancel() {
  ...
  bool retry_is_safe;
  for (int i = 0; i < 3; ++i) {
    rpc_status = backend_client.DoRpc(&ImpalaInternalServiceClient::CancelQueryFInstances,
        params, &dummy, &retry_is_safe);
