...
/// cache (which is roughly 48 bytes per receiver).  TODO: We don't need millisecond
/// precision here.
...
DataStreamMgr::DataStreamMgr(MetricGroup* metrics)
    : sender_response_pool_("data-stream-mgr", "sender-response", 4, 10000,
        [](int thread_id, const TransmitDataCtx& workitem) {
          workitem.context->RespondRpcFailure(
  ...
  maintenance_thread_.reset(new Thread("data-stream-mgr", "maintenance",
          [this]() { this->Maintenance(); }));
}
...
shared_ptr<DataStreamRecvr> DataStreamMgr::CreateRecvr(RuntimeState* state,
  ...
  EarlySendersList waiters;
  {
    ...
    if (it != early_senders_.end()) {
      waiters = move(it->second);
      early_senders_.erase(it);
    }
  }

  for (TransmitDataCtx& ctx: waiters.waiting_senders) {
    // TODO: This should perhaps go into the shared thread pool.
    recvr->AddBatch(move(ctx));
...
void DataStreamMgr::Cancel(const TUniqueId& fragment_instance_id) {
  ...
  while (i != fragment_recvr_set_.end() && i->first == fragment_instance_id) {
    shared_ptr<DataStreamRecvr> recvr;
    bool unused;
    recvr = FindRecvr(i->first, i->second, &unused);
    if (recvr) {
      recvr->CancelStream();
    } else {
...
void DataStreamMgr::Maintenance() {
  while (true) {
    ...
    // First signal all receivers to clean up their pending sender queues.
    ...
    // Second, notify any senders that have been waiting too long for their receiver to
    // appear. As above, keep lock_ held for only a short amount of time.
    vector<TransmitDataCtx> senders;
    {
      int64_t now = MonotonicMillis();
      lock_guard<mutex> l(lock_);
    ...
    int time = MonotonicMillis() - start;
    if (time > 1000) {
      VLOG_QUERY << "DataStreamMgr maintenance tasks complete. Took: "
