...
void RuntimeFilterBank::UpdateFilterFromLocal(int32_t filter_id,
  ...
  if (has_remote_target
    ...
    auto completion_cb = [](const Status& status, UpdateFilterRequestPb* request,
    ...
    Rpc<DataStreamServiceProxy>::Make(
        state_->query_ctx().coord_address, ExecEnv::GetInstance()->rpc_mgr())
        .SetTimeout(MonoDelta::FromSeconds(60))
        .SetMaxAttempts(5)
...
void RuntimeFilterBank::PublishGlobalFilter(
    int32_t filter_id, const BloomFilterPb& filter_pb) {
  lock_guard<mutex> l(runtime_filter_lock_);
  if (closed_) return;
  RuntimeFilterMap::iterator it = consumed_filters_.find(filter_id);
