...
void Coordinator::ExecRemoteFInstance(
  ...
  auto rpc = Rpc<ImpalaInternalServiceProxy>::Make(
  ...
  Status rpc_status =
      rpc.SetTimeout(kudu::MonoDelta::FromSeconds(10))
          .ExecuteWithThriftArgs(
              &ImpalaInternalServiceProxy::ExecPlanFragment, &rpc_params, &thrift_result);
...
void Coordinator::CancelFragmentInstances() {
  ...
  for (InstanceState* exec_state: fragment_instance_states_) {
    ...
    ++num_cancelled;
    TCancelPlanFragmentParams* params = new TCancelPlanFragmentParams();
    params->protocol_version = ImpalaInternalServiceVersion::V1;
    params->__set_fragment_instance_id(exec_state->fragment_instance_id());

    TCancelPlanFragmentResult* res = new TCancelPlanFragmentResult();
    ...
    Rpc<ImpalaInternalServiceProxy>::Make(
        exec_state->impalad_address(), ExecEnv::GetInstance()->rpc_mgr())
        .SetMaxAttempts(5)
        .SetRetryInterval(100)
        .ExecuteWithThriftArgsAsync(&ImpalaInternalServiceProxy::CancelPlanFragmentAsync,
            params, res, completion_cb);
  }
...
void Coordinator::UpdateFilter(const UpdateFilterRequestPB* params) {
  ...
  for (int target_idx: target_fragment_instance_state_idxs) {
    ...
    PublishFilterRequestPB* request = new PublishFilterRequestPB();
    ...
    PublishFilterResponsePB* ptr = new PublishFilterResponsePB();

    // Capture rpc_params here so that when all RPC are complete, the last handler will
    // delete the memory associated with rpc_params.
    auto cb = [remaining_rpcs](const Status& status, PublishFilterRequestPB* request,
      ...
      // filter as well.
      if (remaining_rpcs->Add(-1) != 0) {
        request->release_bloom_filter();
      }
      delete request;
      delete response;
    };
    rpc.ExecuteAsync(&DataStreamServiceProxy::PublishFilterAsync, request, ptr, cb);
  }
