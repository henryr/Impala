...
#include "service/exec_control_service.pb.h"
#include "service/exec_control_service.proxy.h"
...
DEFINE_int32(rpc_exec_plan_fragment_timeout_ms, 120000,
    "Timeout for plan fragment exec RPCs.");
DEFINE_int32(rpc_cancel_plan_fragment_timeout_ms, 10000,
  "Timeout for plan fragment cancellation RPCs");
...
class Coordinator::FilterState {
 public:
  ...
  ProtoBloomFilter* release_bloom_filter() { return bloom_filter_.release(); }
...
void Coordinator::ExecRemoteFInstance(
  ...
  Status rpc_status = rpc.SetTimeout(
      kudu::MonoDelta::FromMilliseconds(FLAGS_rpc_exec_plan_fragment_timeout_ms))
  ...
  for (InstanceState* exec_state: fragment_instance_states_) {
    ...
    auto completion_cb = [exec_state, barrier = &all_rpcs_done, query_id = query_id_]
      ...
      auto notifier = NotifyBarrierOnExit(barrier);
      {
        ...
        } else if (res->status.status_code != TErrorCode::OK) {
    ...
    MonoDelta timeout =
        MonoDelta::FromMilliseconds(FLAGS_rpc_cancel_plan_fragment_timeout_ms);
    rpc.SetMaxAttempts(5).SetRetryInterval(100).SetTimeout(timeout);
    rpc.ExecuteWithThriftArgsAsync(&ExecControlServiceProxy::CancelPlanFragmentAsync,
...
void Coordinator::UpdateFilter(int32_t filter_id, const ProtoBloomFilter& params) {
  ...
  unordered_set<int> target_fragment_instance_state_idxs;
  {
    ...
    state->Disable(filter_mem_tracker_.get());
    DCHECK_EQ(state->bloom_filter(), reinterpret_cast<ProtoBloomFilter*>(NULL));
  }
  ...
  for (int target_idx: target_fragment_instance_state_idxs) {
    ...
    auto rpc = Rpc<DataStreamServiceProxy>::Make(fragment_inst->impalad_address(),
