...
/// discard the batch if the buffer is full. In the first two cases, the sender is
/// notified when the batch has been processed, or added to the batch queue,
...
/// immediately as well. If the batch is added to the receiver's batch queue, the RPC is
/// also responded to immediately. Both of these cases are allow the sender to prepare a
/// new batch while the current one is processed by the receiver, which is critical to
/// good performance.

/// If the batch is rejected, the batch is discarded and sender is added to a per-receiver
/// 'pending sender queue'. The receiver signals senders in this queue to resend the
/// discarded batch when space is available. If the sender sits in the pending sender
/// queue for too long without receiving a response, it may time out on the sender
/// side. To avoid this risk, the receiver periodically responds to all senders in the
/// sender queue, and aims to do so halfway through (i.e. 60s) their timeout period. The
/// assumpyion is that, even taking into account clock rate discrepancies and other
/// delays, the error in notification should not exceed 60s and so the sender will not
/// time out.
///
