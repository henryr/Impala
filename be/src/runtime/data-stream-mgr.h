...
/// stream manager defers replying to the sender until the receiver has finished
/// initialization. If the receiver does not appear within a configurable timeout, the
/// data stream manager notifies the sender directly by returning
...
/// added to the batch queue, respectively. The sender will then send its next batch.
///
...
/// immediately as well. If the batch is added to the batch queue, the RPC is also
/// responded to immediately. Both of these cases are allow the sender to prepare a new
/// batch while the current one is processed by the receiver, which is critical to good
/// performance. If the batch is rejected and the sender is added to the pending sender
/// queue, the sender may time out if it sits in the sender queue for too long. To avoid
/// this risk, the receiver periodically responds to all senders in the sender queue, and
/// aims to do so halfway through (i.e. 60s) their timeout period. The assumpyion is that,
/// even taking into account clock rate discrepancies and other delays, the error in
...
class DataStreamMgr {
 public:
  /// The RPC timeout for a TransmitData() call. Receivers will try to process pending
  /// senders before this timeout elapses in order to avoid timeouts during busy periods.
  ...
  /// Adds a row batch to the recvr identified by fragment_instance_id/dest_node_id if the
  /// recvr has not been cancelled. sender_id identifies the sender instance from which
  /// the data came. If the recvr has not yet prepared 'payload' is queued until it
  /// arrives, or is timed out. If the recvr has already been torn-down (within the last
  /// STREAM_EXPIRATION_TIME_MS), the sender will be responded to immediately. Otherwise
  ...
  void AddData(const TUniqueId& fragment_instance_id, TransmitDataCtx&& payload);
 ...
 private:
  ...
  ThreadPool<TransmitDataCtx> sender_response_pool_;
  ...
  /// Enqueues a pending sender to be replied to by a thread in sender_response_pool_.
  void EnqueueSenderReject(TransmitDataCtx&& ctx);
  ...
  struct EarlySendersList {
    ...
    std::vector<TransmitDataCtx> waiting_senders;
