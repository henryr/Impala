...
class DataStreamSender::Channel :
 ...
 private:
  ...
  /// same shared_ptr. Set in Init(), and reset in ReleaseResources().
...
Status DataStreamSender::Channel::SendBatch(RowBatch* batch) {
  auto proto_batch = proto_batches_[proto_batch_idx_];
...
Status DataStreamSender::Channel::SendSerializedBatch(
  ...
  // expire.
  auto cb = [self_ptr = weak_ptr<DataStreamSender::Channel>(self_),
