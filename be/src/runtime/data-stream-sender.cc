...
class DataStreamSender::Channel : public CacheLineAligned {
 ...
 private:
  ...
  // Lock with rpc_done_cv_ protecting rpc_in_flight_
  mutex rpc_thread_lock_;

  // signaled when rpc_in_flight_ is set to true.
...
Status DataStreamSender::Channel::SendBatch(RowBatchPb* batch) {
  ...
  auto cb = [this](const Status& status, TransmitDataRequestPb* request,
    ...
    {
      lock_guard<SpinLock> l(rpc_status_lock_);
      Status rpc_status = status.ok() ? FromKuduStatus(controller->status()) : status;

      if (!rpc_status.ok()) {
        last_rpc_status_ = rpc_status;
      } else if (response->status().status_code() != TErrorCode::OK) {
        last_rpc_status_ = Status(response->status());
      } else {
        num_data_bytes_sent_.Add(RowBatch::GetBatchSize(request->row_batch()));
        VLOG_ROW << "incremented #data_bytes_sent=" << num_data_bytes_sent_.Load();
      }
    }
...
Status DataStreamSender::Prepare(RuntimeState* state, MemTracker* parent_mem_tracker) {
  ...
  for (Channel* channel : channels_) RETURN_IF_ERROR(channel->Init());
  return Status::OK();
}
