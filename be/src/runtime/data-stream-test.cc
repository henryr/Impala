...
using rpc::ImpalaInternalServiceProxy;
using rpc::ExecPlanFragmentRequestPB;
using rpc::ExecPlanFragmentResponsePB;
using rpc::ReportExecStatusRequestPB;
using rpc::ReportExecStatusResponsePB;
using rpc::CancelPlanFragmentRequestPB;
using rpc::CancelPlanFragmentResponsePB;
using rpc::TransmitDataRequestPB;
using rpc::TransmitDataResponsePB;
using rpc::PublishFilterRequestPB;
using rpc::PublishFilterResponsePB;
using rpc::UpdateFilterRequestPB;
using rpc::UpdateFilterResponsePB;

class ImpalaTestBackend : public rpc::ImpalaInternalServiceIf {
 public:
  ImpalaTestBackend(const scoped_refptr<kudu::MetricEntity>& entity,
      const scoped_refptr<kudu::rpc::ResultTracker> tracker): ImpalaInternalServiceIf(entity, tracker) {}

  void set_datastream_mgr(DataStreamMgr* mgr) { mgr_ = mgr; }
  virtual ~ImpalaTestBackend() {}

  virtual void ExecPlanFragment(
      const ExecPlanFragmentRequestPB* request, ExecPlanFragmentResponsePB* response, RpcContext* context) {}

  virtual void ReportExecStatus(
      const ReportExecStatusRequestPB* request, ReportExecStatusResponsePB* response, RpcContext* context) {}

  virtual void CancelPlanFragment(
      const CancelPlanFragmentRequestPB* request, CancelPlanFragmentResponsePB* response, RpcContext* context) {}

  virtual void TransmitData(const TransmitDataRequestPB* request, TransmitDataResponsePB* response, RpcContext* context) {
    TUniqueId id;
    id.__set_hi(request->dest_fragment_instance_id().hi());
    id.__set_lo(request->dest_fragment_instance_id().lo());

    if (!request->eos()) {
      mgr_->AddData(id, request->dest_node_id(),
          request->row_batch(), request->sender_id()).ToProto(response->mutable_status());
    } else {
      mgr_->CloseSender(id, request->dest_node_id(),
          request->sender_id()).ToProto(response->mutable_status());
    }
    context->RespondSuccess();
  }

  virtual void PublishFilter(const PublishFilterRequestPB* request, PublishFilterResponsePB* response, RpcContext* context) {

  }

  virtual void UpdateFilter(const UpdateFilterRequestPB* request, UpdateFilterResponsePB* response, RpcContext* context) {
...
class DataStreamTest : public testing::Test {
 protected:
  ...
  void StartBackend() {
    ImpalaTestBackend* backend;
    ExecEnv::GetInstance()->rpc_mgr()->Start(FLAGS_port);
    ExecEnv::GetInstance()->rpc_mgr()->RegisterService(&backend);
    backend->set_datastream_mgr(stream_mgr_);
  }
...
TEST_F(DataStreamTest, CloseRecvrWhileReferencesRemain) {
  ...
  EXPECT_TRUE(ExecEnv::GetInstance()->rpc_mgr()->GetProxy(MakeNetworkAddress("localhost", FLAGS_port),
          &proxy).ok());
  ...
  EXPECT_OK(FromKuduStatus(controller.status()));
