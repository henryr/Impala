...
RowBatch::RowBatch(
    const RowDescriptor& row_desc, const RowBatchPB& input_batch, MemTracker* mem_tracker)
  ...
  if (input_batch.compression_type() != THdfsCompression::NONE) {
    ...
    Status status = Codec::CreateDecompressor(NULL, false, (THdfsCompression::type)input_batch.compression_type(),
        &decompressor);
  ...
  int tuple_idx = 0;
  for (int i = 0; i < input_batch.tuple_offsets().size(); ++i) {
    if (input_batch.tuple_offsets(i) == -1) {
      tuple_ptrs_[tuple_idx++] = nullptr;
    } else {
      tuple_ptrs_[tuple_idx++] = reinterpret_cast<Tuple*>(tuple_data + input_batch.tuple_offsets(i));
    }
  }
...
Status RowBatch::Serialize(RowBatchPB* output_batch) {
  return Serialize(output_batch, UseFullDedup());
}

Status RowBatch::Serialize(RowBatchPB* output_batch, bool full_dedup) {
  output_batch->Clear();
  output_batch->set_num_rows(num_rows_);
  output_batch->set_compression_type(THdfsCompression::NONE);
  row_desc_.ToProto(output_batch);
  ...
  if (size > 0) {
    ...
    const uint8_t* input = reinterpret_cast<const uint8_t*>(output_batch->mutable_tuple_data()->c_str());
    ...
    if (LIKELY(compressed_size < size)) {
      compression_scratch_.resize(compressed_size);
      output_batch->mutable_tuple_data()->swap(compression_scratch_);
...
void RowBatch::SerializeInternal(int64_t size, DedupMap* distinct_tuples,
  ...
  // TODO (KRPC): Fix this
  // output_batch->mutable_tuple_offsets()->reserve(num_rows_ * num_tuples_per_row_);
  ...
  for (int i = 0; i < num_rows_; ++i) {
    ...
    for (int j = 0; desc != row_desc_.tuple_descriptors().end(); ++desc, ++j) {
      ...
      if (UNLIKELY(tuple == NULL)) {
        ...
        int prev_row_idx = output_batch->tuple_offsets_size() - num_tuples_per_row_;
        output_batch->add_tuple_offsets(output_batch->tuple_offsets(prev_row_idx));
        continue;
        ...
        if (*dedupd_offset != offset) {
          ...
          output_batch->add_tuple_offsets(*dedupd_offset);
          continue;
        }
      }
...
int RowBatch::GetBatchSize(const RowBatchPB& batch) {
