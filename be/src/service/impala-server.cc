...
void ImpalaServer::PrepareQueryContext(TQueryCtx* query_ctx) {
  ...
  query_ctx->__set_data_svc_port(FLAGS_state_store_subscriber_port);
...
void ImpalaServer::AddLocalBackendToStatestore(
  ...
  local_backend_descriptor.__set_data_svc_port(FLAGS_state_store_subscriber_port);
  IpAddr ip;
...
Status ImpalaServer::Init(int32_t beeswax_port, int32_t hs2_port) {
  ...
  int32_t service_port = exec_env_->data_svc_port();
...
void ImpalaServer::UpdateFilter(int32_t filter_id, const TUniqueId& query_id,
  ...
  GetClientRequestState(query_id, false);
