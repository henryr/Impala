...
void DataStreamService::TransmitData(const TransmitDataRequestPb* request,
  ...
  if (status.ok()) {
    ...
    status = ExecEnv::GetInstance()->stream_mgr()->AddData(finst_id, payload);
  }

  // Something went wrong in a way that isn't handled by AddData().
  if (!status.ok()) {
...
void DataStreamService::PublishFilter(const PublishFilterRequestPb* request,
  ...
  if (qs.get() != nullptr) {
    ...
    if (fis != nullptr) {
      ...
      // TODO: Check return
      context->GetInboundSidecar(proto_filter.header.directory_sidecar_idx(),
          &proto_filter.directory);
      fis->PublishFilter(request->filter_id(), proto_filter);
    }
  }
...
void DataStreamService::UpdateFilter(const UpdateFilterRequestPb* request,
  ...
  if (!filter.header.always_true()) {
    context->GetInboundSidecar(filter.header.directory_sidecar_idx(), &filter.directory);
  }
