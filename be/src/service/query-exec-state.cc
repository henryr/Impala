...
Status ImpalaServer::QueryExecState::ExecQueryOrDmlRequest(
  ...
  if (!query_exec_request.__isset.desc_tbl) {
    // If desc_tbl is not set, query has SELECT with no FROM. In that case, the query can
    // only have a single fragment, and that fragment needs to be executed by the
    // coordinator. This check confirms that.
    DCHECK_EQ(schedule_->GetNumFragmentInstances(), 1);
    DCHECK(schedule_->GetCoordFragment() != nullptr);
  }
